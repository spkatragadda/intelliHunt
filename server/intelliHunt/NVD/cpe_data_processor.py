...
class CPEDataProcessor:
    ...
    def __init__(self, cpe_data_path: str = None):
        ...
        self.endpoint = "https://services.nvd.nist.gov/rest/json/cves/2.0?cpeName="
    ...
    def _make_request(self, cpe):
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(f"Making request for cpe: {cpe}")
                response = self.session.get(self.endpoint+cpe, timeout=self.timeout)
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.error(f"All {self.max_retries} attempts failed for cpe: {cpe}")
                    return None
    ...
    def extract_software_stack(self) -> Dict[str, List[str]]:
        ...
        # lookup on cpe names to get cve descriptions
        # all_cpe_names = list(set(all_cpe_names))
        # all_descriptions = []
        # for cpe in all_cpe_names[0:10]:
        #     all_descriptions.append(self._make_request(cpe))
        # software_stack['cve_description'] = all_descriptions
    ...
    def generate_crew_input(self) -> Dict[str, Any]:
        ...
        crew_input = {
            ...
            # 'descriptions':{
            #     'all_descriptions' : software_stack['cve_description']
            # }
        }
